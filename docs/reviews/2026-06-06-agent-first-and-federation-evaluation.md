# Charter as an Agent-First Tool — Evaluation & Federation Readiness
...
and cross-repo reading of the `~/weft` hub + Filigree / Clarion(Loomweave) /
Wardline / Legis.
...
**Charter's agent-first *design* is genuinely strong; its agent-first *mechanics*
...
text." What's missing is the operational plumbing that lets an agent run Charter *in
a loop without corrupting it or getting stuck* — recoverable errors, idempotent
write verbs, safe concurrency — and the federation adapters that make Charter a
...
| 6 | `loom.*` → `weft.*` naming drift (54 schema sites + `~/loom` pointers) vs. the canonical `~/weft` hub | Federation hygiene | **P2** (time-boxed) |
| 7 | Federation adapters absent — SEI snapshot not stored on trace links; no Clarion/Filigree/Wardline/Legis binding; `weft.charter.preflight_facts.v1` producer not built | Deferred-by-design | tracked, P1–P2 on roadmap |
...
- **Declared authority boundary.** `charter_project_context_get` returns
  ...
  The agent can know what Charter will and won't do before calling.
...
already ships, and they are what most degrade my (the agent's) ability to use Charter.
...
`hint="Refresh local Charter state and retry."`, and **no `details`**
...
Charter is materially behind its sibling Filigree here, which ships an
...
- If Charter is **single-writer-per-repo by design**, say so explicitly in an ADR —
...
`{initialized, project_key, schema_version, db_path}`; there is no `charter
capabilities`, no `charter contracts`, no machine-readable command/output catalogue
...
schema-publication slice (real JSON Schema per `weft.charter.*.v1`).
...
`next_action` carries an executable `command`, a `charter work next` /
`session_context` surface, and a bridge that promotes a Charter gap into a Filigree
...
## 4. Federation wiring (Filigree, Wardline, Clarion/Loomweave, Legis)
...
opaque peer ids for issues/findings/attestations. Charter is the federation's
read-only **obligations** consumer. All four Charter adapters are currently
**unimplemented** (no peer source files under `src/charter/`) — this is expected at
pre-alpha, but here is what "first-class" requires:

### 4.1 Naming drift — reconcile while pre-alpha (P2, sequencing decision)
Verified: the hub is `~/weft` (`~/loom` does not exist); the canonical Charter
contract is `weft.charter.preflight_facts.v1` (`~/weft/contracts-index.md`); yet
Charter ships **`weft.charter.*`** schema names (54 sites incl. every contract
fixture) and `concept.md`/ADR-001/005/006 point at `~/loom/*`. Renaming is a breaking
contract change, but Charter is pre-alpha with adapters pending — **now is the window,
before any external consumer locks onto `weft.charter.*`.** Maintainer owns the
...
Charter's `trace_links` store a generic opaque `(to_kind, to_id)` with no SEI
snapshot. ADR-005 specifies storing a Clarion code target as an **opaque SEI plus a
snapshot tuple**: locator, content hash, lineage status, Clarion version, observed-at.
Adopt the suite's **two-axis freshness**: identity (ALIVE/ORPHANED, owned by
Clarion's `resolve_sei`) and content (FRESH/STALE, owned by Charter's stored
...
Clarion at propose-time, mark links stale/orphaned on refresh, and degrade to fragile
file/symbol refs when Clarion is absent (never destructively mutate on `PEER_ABSENT`).
**Consumers MUST NOT parse the SEI.**

> **Escalate, don't guess — SEI prefix conflict.** The LOCKED standard
> (`~/weft/sei-standard.md`) and Filigree ADR-017 use `clarion:eid:`; Clarion's
> *shipping* code mints `loomweave:eid:` (`clarion/.../sei.rs`), and `concept.md:393`
> shows a third stale form (`sei:01HX…`). A fail-closed "is-this-already-an-SEI" guard
> that hard-codes the wrong reserved prefix mishandles backfilled values. Charter must
...
### 4.3 The one outbound producer — `weft.charter.preflight_facts.v1` → Legis
Charter's only thing it *produces* to a peer (ADR-006): a facts envelope (kinds:
`requirement_touched`, `requirement_verification_stale/_missing`, `baseline_drift`,
`trace_gap`, `open_linked_work`, `active_finding_linked`, …). Charter may classify a
...
This directly informs the D1 work just landed (see §5): **Charter's actor registry is
a Charter-*local*, project-scoped (by DB file) attestation-authority construct — it
does NOT constitute a shared identity oracle and so does not violate the doctrine.**
The right reconciliation is to align Charter's actor semantics with ADR-012
...
1. **`verified_actor` is a better genesis anchor than "first-come."** The open D1
   follow-up `charter-59e4af3aaf` (seed the genesis attester at `init`) currently
   ...
   user who runs `charter init` is the natural, transport-verified genesis attester.
   *(Captured as a comment on `charter-59e4af3aaf`.)*
2. **Charter-local, not federated.** State plainly in any federation work that the
...
The §4 adapters are framed as plumbing ("Charter binds to peer X"). The real
justification for building them is the *emergent* capability that no single tool can
produce. Unifying frame: **Charter owns the obligation; Loomweave owns the code
...
integration below is a new *channel* into Charter's obligation model — and the
highest-value items are cross-channel contradiction detection.

> Naming: **Clarion is now Loomweave** (confirmed by maintainer 2026-06-06). The code
> identity authority is referred to as Loomweave below; reinforces §4.1.

### 6.1 Loomweave (code identity / graph) — turns traceability from a liability into an asset
- **Impact analysis — the deferred headline product value (`concept.md:8`).** A diff →
  Loomweave resolves changed entities to SEIs and their graph neighbourhood →
  reverse-lookup the trace links targeting those SEIs → **the exact set of
  requirements impacted by a change.** Charter literally cannot answer its own
  opening question without Loomweave's code graph. This is the single biggest
  value-add in the suite.
- **Refactor-resilient traceability.** Because links key on opaque SEI (identity-stable
  across renames, lineage-tracked by Loomweave), `validate_token`→`verify_token` does
...
- **Change-scoped verification staleness (more precise than today's model).** Charter
  currently staleness-checks on *requirement version*. With Loomweave, it can stale on
...
- **Orphan + untraced-surface detection (bidirectional).** Loomweave says an entity is
  ORPHANED → the requirement that claimed satisfaction-by-that-code reverts to
  at-risk/unverified. Conversely, Loomweave entities with no accepted requirement trace
...
- **Assisted trace proposals (keeps traceability cheap — the product's primary goal,
  `concept.md:27`).** When an agent edits in a requirement's Loomweave neighbourhood,
  Charter can *propose* (never accept) a trace link, so coverage grows during ordinary
...
- **Security/safety requirements get machine evidence, not just human attestation.**
  Charter's verification methods are {test, analysis, inspection, manual}. Wardline is
...
- **Counter-evidence with a distinct authority — ties directly to D1.** Most evidence
  ...
  boundary is violated). It feeds Charter's existing `failing`/`inconclusive` statuses
...
- **Finding accountability (both directions).** A SARIF finding floating loose is
  ...
  verification gap Charter can surface.
- **High-risk-waiver surfacing (composes with D1 waivers).** A human waiver (D1 path)
  ...
  review must see. Charter can mark "waived-over-active-finding" as a distinct,
...
- **The dossier as the federation join point (killer agent feature).** The dossier
  ...
  (Loomweave SEIs + freshness) + its findings (Wardline) + its work (Filigree) + its
  governance refs (Legis). "Everything I need before touching this surface" in one
  call. Charter is uniquely positioned to host it because it owns the obligation that
  ties the others together.
- **Enriched pre-commit preflight to Legis.** `weft.charter.preflight_facts.v1` becomes
  genuinely decisive when composed: "this commit touches REQ-AUTH entities (Loomweave)
  whose verification is stale (Charter) **and** that carry an active taint finding
  (Wardline) → `block_candidate`." No single tool produces that signal.
- **Composite release readiness.** Baseline diff (Charter) + entity lineage drift
  (Loomweave) + open findings on baselined code (Wardline) + linked open work
...
- **Contradiction detection — the highest-value governance capability.** Three-way
  consistency: requirement says X; Loomweave entity Y is traced to X; Wardline says Y
  ...
  Charter computes a **contradiction**. Catching "we believe inconsistent things" is
...
| `loom.*`→`weft.*` namespace + `~/loom`→`~/weft` pointer reconciliation (pre-alpha window) | task, P2 | §4.1 |
| Trace-link SEI snapshot tuple + two-axis freshness (Clarion consumer adapter) | feature, P1 | §4.2 |
| `weft.charter.preflight_facts.v1` producer (Legis preflight) | feature, P1 | §4.3 |

Existing related: `charter-59e4af3aaf` (genesis seeding — update with `verified_actor`),
`charter-606b6e0a94` (register_actor re-registration polish).
