...
**Context**: Plainweave's highest product risk is agent-generated traceability that looks accepted but is not trustworthy.

## Summary

Plainweave will use a fixed trace-link ontology with canonical relation direction and explicit authority/freshness states. Agents may propose traceability freely, but accepted traceability is a separate state.

## Context

Requirements traceability fails when links are inconsistent or unverifiable. Agentic workflows increase the risk because agents can create many plausible links quickly. Plainweave must enable this productivity without creating false certainty.
...
**Why rejected**: Plainweave needs machine-usable traceability, not prose tags.
