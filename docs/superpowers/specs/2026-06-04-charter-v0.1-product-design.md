...
Shuttle, later, executes scoped changes.
```
...
- Git/CI gate decisions, sign-offs, attestations, or override trails:
  Legis owns these.
- Transactional scoped change execution and rollback provenance: future Shuttle
  owns these.
...
charter criterion remove

charter link
charter propose-link
charter accept-link
charter reject-link
charter unlink
charter trace
...
charter federation resolve-entity
charter federation preflight-facts
```
...
### Standalone Requirement Authoring

1. `charter add "Reject expired bearer tokens" --type security --criticality high`
2. `charter criterion add REQ-AUTH-017 "... receives HTTP 401 ..."`
3. `charter approve REQ-AUTH-017`
4. `charter verify REQ-AUTH-017 --method test --evidence tests/test_auth.py::test_expired_token_rejected`
5. `charter baseline create release-1.0`
...
- Future Shuttle scoped-change obligation handoff.
